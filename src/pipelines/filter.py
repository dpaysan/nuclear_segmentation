...
from src.selection.filtering import ConfocalShiftFilter, DeadCellFilter, ObjectPropertyFilter, AreaFilter, AspectRatioFilter
...
class FilterPipeline(object):
    ...
    def get_object_properties(self, kernel_size: int = 3, iterations: int = 3):
        ...
        object_properties = regionprops(labeled_image, intensity_image=np.squeeze(self.image))
    ...
    def plot_images(self, image_type:str='filtered'):
        if image_type == 'filtered':
            plot_3d_images_as_map(self.filtered_images)
        elif image_type == 'filtered_out':
            plot_3d_images_as_map(self.filtered_out_images)
        else:
